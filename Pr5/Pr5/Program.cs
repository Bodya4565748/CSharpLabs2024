...
public delegate void MageActionHandler(Mage mage, string action, string spell);

public abstract class Mage
{
    public event MageActionHandler MageAction; // Подія для сповіщення про дії мага
    ...
    protected void OnMageAction(Mage mage, string action, string spell)
    {
        MageAction?.Invoke(mage, action, spell); // Виклик події, якщо вона не пуста
    }
}
...
public class FireMage : Mage
{
    ...
    public override void Attack(string spell)
    {
        ...
        OnMageAction(this, "Attack", spell); // Виклик події для атаки
    }
    ...
    public override void Defend(string spell)
    {
        ...
        OnMageAction(this, "Defend", spell); // Виклик події для захисту
    }
}
...
class Program
{
    static void Main()
    {
        ...
        mage1.MageAction += (mage, action, spell) =>
        {
            Console.WriteLine($"{mage.Name} виконує дію '{action}' з заклинанням '{spell}'.");
        };

        mage2.MageAction += (mage, action, spell) =>
        {
            Console.WriteLine($"{mage.Name} виконує дію '{action}' з заклинанням '{spell}'.");
        };
